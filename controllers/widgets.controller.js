...
result.getAll = async (req, res)=>{
    ...
    try{
        const rsDetail = await widgetsModel.getAllFromDB(params);
...
result.getNavigationsDetail = async (req,res)=>{
    ...
    let mysql = null;
    try{
        const rsDetail = await widgetsModel.getNavigationsDetailFromDB(id);
        baseResponse.data = rsDetail.data;
        baseResponse.success = rsDetail.success;
        baseResponse.message = rsDetail.message;
        baseResponse.responseCode = rsDetail.responseCode;
...
result.addWidgets = async (req,res) =>{
    let params = {...req.body};
    let mysql = null;
    
    //ให้ check อะไรก่อนมั้ย
    try{
        const rsAdd = await widgetsModel.addWidgetsFromDB(params);
...
result.deleteWidgets = async(req,res) =>{
    let mysql = null;
    const id = req.params.id;
    try {
        const reDetail = await widgetsModel.deleteWidgetsfromDB(id);
...
result.updateWidgets = async function(req,res){
    let mysql = null;
    const id = req.params.id;
    const params = {...req.body};
    ...
    try {
        const rsAdd = await widgetsModel.updateWidgetseFromDb(params,id);
        baseResponse.message = rsAdd.message;
        baseResponse.data = rsAdd.data;
        baseResponse.success = rsAdd.success;
        baseResponse.responseCode = rsAdd.responseCode;
...
result.updateWidgeteOrdering = async function(req,res){
    let mysql = null;
    const id = req.params.id;
    ...
    try {
        const rsAdd = await Promise.all(
            widgets.map(async (wid,index) => {
                wid.ordering_count = index;
                await widgetsModel.updateWidgetOrderingFromDB(wid,wid.id);
