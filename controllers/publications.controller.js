import baseResponse from "../helpers/base-response.helper";
import mysqlConnector from "../db/mysql-connector";
import publicationsImageModel from '../model/publicationsImage'
import publicationsModel from '../model/publications'
import { randomBytes } from "crypto";
const result = {};

result.getAll = async (req, res)=>{
    let mysql = null;
    const params = req.body;
    try{
        const rsDetail = await publicationsModel.getAllFromDB(params);
        let rests = [];
        if(rsDetail.data){
            rests  = await Promise.all(
                rsDetail.data.map(async (item) => { 
                        
                        
                        //console.log('item',item);
                        return item;
                  
                })
            );
        }
        baseResponse.message = rsDetail.message;
        baseResponse.success = rsDetail.success;
        baseResponse.responseCode = rsDetail.responseCode;
        baseResponse.data = rests;
        return res.status(baseResponse.responseCode).json(baseResponse);

    }catch(error){
        console.log('er',error);
        baseResponse.message = error.message;
        baseResponse.success = error.success;
        baseResponse.responseCode = 400;
        baseResponse.data = undefined;
        return res.status(baseResponse.responseCode).json(baseResponse);
    }finally{
        if(mysql){
            await mysql.release();

        }

    }
}

result.getPublicationsDetail = async (req,res)=>{
    const slug = req.params.slug;
    
    let mysql = null; 
    try{
        const rsDetail = await publicationsModel.getpublicationsDetailFromDB(slug);
        let images = [];
        rsDetail.data.galleries = [];
        if(rsDetail.data){
         
           
            let params = {
                portfolio_id :rsDetail.data.id
            }
           
            images  =  await publicationsImageModel.getAllFromDB(params);
            rsDetail.data.galleries = images.data;
        }
       
        baseResponse.data = rsDetail.data;
        baseResponse.success = rsDetail.success;
        baseResponse.message = rsDetail.message;
        baseResponse.responseCode = rsDetail.responseCode;
    }catch(error){
        baseResponse.message = error.message;
        baseResponse.success = error.success;
        baseResponse.responseCode = error.responseCode;
        baseResponse.data = undefined;
    }finally{
        if(mysql){
            await mysql.release();

        }
    }
    return res.status(baseResponse.responseCode).json(baseResponse);
}

result.addPublications = async (req,res) =>{
    const params = {...req.body};
    const files = {...req.files};
    let mysql = null;
    
    
    try{
        const rsDetail = await publicationsModel.getpublicationsCheckFromDB(params);
        if (rsDetail.success == true){
            if(files){
                if(files.bannerFile){
                    params.banner_image  = await publicationsModel.uploadImages(files.bannerFile,params,req.ref);
                  
                }
                if(files.publicationsFile){
               
                    params.cover_image  = await publicationsModel.uploadImages(files.publicationsFile,params,req.ref);
                }
                if(files.logoFile){
               
                    params.logo_image  = await publicationsModel.uploadImages(files.logoFile,params,req.ref);
                }
                
               
            }
           
            const rsAdd = await publicationsModel.addpublicationsFromDB(params);
           
      
        
           
        
            baseResponse.data = rsAdd.data;
            baseResponse.success = rsAdd.success;
            baseResponse.message = rsAdd.message;
            baseResponse.responseCode = rsAdd.responseCode;
        }    
        else{
            baseResponse.success = false;
            baseResponse.responseCode = 200;
            baseResponse.message = rsDetail.message;
        }

    }catch(error){
        console.log('error',error);
        baseResponse.message = error.message;
        baseResponse.success = error.success;
        baseResponse.responseCode = error.responseCode;
        baseResponse.data = undefined;
    }finally{
        if(mysql){
            await mysql.release();
        }
    }
   
    return res.status(baseResponse.responseCode).json(baseResponse);
}

result.deletePublications = async(req,res) =>{
    let mysql = null;
    const id = req.params.id;
    console.log('id',id);
    try {
      
        const reDetail = await publicationsModel.deletepublicationsfromDB(id);
        baseResponse.message = reDetail.message;
        baseResponse.data = reDetail.data;
        baseResponse.success = reDetail.success;
        baseResponse.responseCode = reDetail.responseCode;

  
    } catch (error) {
        baseResponse.message = error.message;
        baseResponse.success = error.success;
        baseResponse.responseCode = error.responseCode;
        baseResponse.data = undefined;

  
    } finally {
        if(mysql){
            await mysql.release();

        }
    }
    return res.status(baseResponse.responseCode).json(baseResponse);
}

result.updatePublications = async function(req,res){
    let mysql = null;
    const id = req.params.id;
    const params = {...req.body};
    const files = {...req.files};
    const rsCheck = await publicationsModel.getPublicationsCheckUniqueFromDB(params,id);
   
    if (rsCheck.success == true){
        if(files){
            if(files.bannerFile){
                params.banner_image  = await publicationsModel.uploadImages(files.bannerFile,params,req.ref);
              
            }
            if(files.publicationsFile){
           
                params.cover_image  = await publicationsModel.uploadImages(files.publicationsFile,params,req.ref);
            }
            if(files.logoFile){
               
                params.logo_image  = await publicationsModel.uploadImages(files.logoFile,params,req.ref);
            }
            
           
        }
    try {
     
       let rsAdd =  await publicationsModel.updatePublications(params,id);
       let cat = [];
      
        
       
        baseResponse.message = rsAdd.message;
        baseResponse.data = cat;
        baseResponse.success = rsAdd.success;
        baseResponse.responseCode = rsAdd.responseCode;

  
    } catch (error) {
        baseResponse.message = error.message;
        baseResponse.success = error.success;
        baseResponse.responseCode = error.responseCode;
        baseResponse.data = undefined;

  
    } finally {
        if(mysql){
            await mysql.release();

        }
    }
}
    return res.status(baseResponse.responseCode).json(baseResponse);
}
result.uploadImages = async function(req,res){
    const files = {...req.files};
    console.log('files',files);
    baseResponse.message = 'test';
    baseResponse.success = true;
    baseResponse.responseCode = 200;
    baseResponse.data = undefined;
    return res.status(baseResponse.responseCode).json(baseResponse);
}
export default result;
